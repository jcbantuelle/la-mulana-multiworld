...
impl Verifier {
    pub fn verify_install() -> Result<LaMulanaConfig, String> {
        ...
        Self::verify_game_files(lm_config.clone())?;
    ...
    fn verify_game_files(lm_config: LaMulanaConfig) -> Result<(), String> {
        ...
        let files_to_verify = if seeds_dir_exists {
            [
                Self::verify_lm_file(SOURCE_RCD_PATH.to_string(), lm_config.rcd_digest),
                Self::verify_lm_file(SOURCE_DAT_PATH.to_string(), lm_config.dat_digest),
                Self::verify_lm_file(SOURCE_EFFECTS_PATH.to_string(), lm_config.effects_digest)
            ]
        } else {
            [
                Self::verify_lm_file(ORIGINAL_RCD_PATH.to_string(), lm_config.rcd_digest),
                Self::verify_lm_file(ORIGINAL_DAT_PATH.to_string(), lm_config.dat_digest),
                Self::verify_lm_file(ORIGINAL_EFFECTS_PATH.to_string(), lm_config.effects_digest)
            ]
        };
